...
// Same logic to get the fulle rror chain on `Debug`
        ...
        match subscriber {
            Ok(subscriber) => {
                email_client
                    ...
                    .await
                    // If static i.e "oh no!", can use `.context` else if it can have a
                    // runtime cost i.e function, use `with_context`
                    // format! allocates memory on heap and with `.context`
                    // it would be allocating that memory with every email sent
                    .with_context(|| {
...
fn basic_authentication(headers: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    ...
    let decoded_bytes = base64::engine::general_purpose::STANDARD
        ...
        .context("Failed to base64-decoded 'Basic' credentials.")?;
